...
# Используем threading для async_mode, чтобы не было ошибок Eventlet на Python 3.13
...
def api_chats():
    ...
    result=[]
    for c in chats:
        members = [{'id': m.id, 'username': m.username, 'avatar': m.avatar} for m in c.members if m.id != u.id]
...
def api_send_message():
    ...
    media = d.get('media')
    if not chat_id or not (text or media): 
        return jsonify({'error':'empty'}),400
    chat = Chat.query.get_or_404(chat_id)
    if u not in chat.members: 
        abort(403)
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
    # В проде Render без Eventlet используем threading
